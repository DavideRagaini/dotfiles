...
def float_cycle(qtile, forward: bool):
    ...
    if forward: floating_window_index += 1
    else:
...
# Grow windows
    ...
    Key([mod,"shift"], "Tab", lazy.run_extension(WindowList(
...
# Applications
    ...
    Key([mod, "shift"], "Insert", lazy.spawn('notify-send \" Clipboard contents:\" \"$(xclip -o -selection clipboard)\"'), desc = "Clipboard Current"),
# Media
...
#  󰇮   󰈙     
groups = [
    Group(name='1', position=1, label='1', layout='max', exclusive=True,
          matches=[Match(wm_class='Emacs')]
          ),
    Group(name='2', position=2, label='2', layout='monadtall', exclusive=True,
          matches=[Match(wm_class='qutebrowser'), Match(wm_class='LibreWolf')]
        ),
    Group(name='3', position=3, label='3', layout='max', exclusive=True,
          matches=[Match(wm_instance_class='Zathura'), Match(wm_class='ebook-viewer')]
          ),
    Group(name='4', position=4, label='4', layout='max' ,
          matches=[Match(wm_class='VirtualBox Machine')]
          ),
    Group(name='5', position=5, label='5', layout='bsp' ,
          # matches=[Match(title='irc'), Match(wm_class='Skype'), Match(wm_class='microsoft teams - preview'), Match(wm_class='discord')]
          ),
    Group(name='6', position=6, label='6', layout='bsp',
          # matches=[Match(title='updates'), Match(wm_class='Gnome-system-monitor'), Match(wm_class='VirtualBox Manager')]
          ),
    Group(name='7', position=7, label='7', layout='bsp',
          # matches=[Match(title='updates'), Match(wm_class='Gnome-system-monitor'), Match(wm_class='VirtualBox Manager')]
          ),
    Group(name='8', position=8, label='8', layout='bsp',
          # matches=[Match(title='updates'), Match(wm_class='Gnome-system-monitor'), Match(wm_class='VirtualBox Manager')]
          ),
    Group(name='9', position=9, label='9', layout='bsp', exclusive=True,
...
groups.append(ScratchPad('scratchpad', [
    DropDown('Dropdown', terminal + " -e tmux new-session -A -s 'dropdown'",
             width=0.9, height=0.9, x=0.04, y=0.04, opacity=0.95),
    DropDown('Agenda', 'eagenda',
             width=0.8, height=0.8, x=0.1, y=0.1, opacity=0.9),
    DropDown('file_manager', file_manager,
             width=0.8, height=0.8, x=0.1, y=0.1, opacity=0.9),
    DropDown('process_viewer', process_viewer,
             width=0.8, height=0.8, x=0.1, y=0.1, opacity=0.9),
    DropDown('news', terminal + " -e newsboat",
             width=0.8, height=0.8, x=0.1, y=0.1, opacity=0.9),
    DropDown('music', terminal + " -e mp",
             width=0.8, height=0.8, x=0.1, y=0.1, opacity=0.9),
    DropDown('mixer', terminal + " -e pulsemixer",
             width=0.6, height=0.3, x=0.2, y=0.2, opacity=0.9),
    DropDown('calculator', terminal + " -e wcalc -P -1 -c -q --ints -C -p -r --remember",
             width=0.8, height=0.8, x=0.1, y=0.1, opacity=0.9),
]))
keys.extend([
    ...
    Screen(
        top = bar.Bar(
            ...
            widget.WindowName(
                ...
                widget.Chord(
                        chords_colors = {'launch': (foregroundColor, foregroundColor),},
               ...
               widget.Prompt(
                ...
                widget.Clock(format=' %V %a %d/%B/%y',
                        update_interval = 86400,
                        foreground = colors[10],
                        background = backgroundColor,
                        ),
                widget.Clock(format=' %T',
                        update_interval = 1,
                        foreground = colors[5],
                        background = backgroundColor,
                        ),
